...
\# Sau khi login, nhận access\_token từ response
...
Token hết hạn sau \*\*24 giờ\*\* (cấu hình: `JWT\_EXPIRATION\_MS=86400000`).
...
\- `order\_id` (unique, VNM + timestamp)
...
\- \*\*UNIQUE:\*\* (product\_id, user\_id) — một user chỉ review một sản phẩm một lần
...
spring.datasource.url=jdbc:mysql://localhost:3306/vinamilk?createDatabaseIfNotExist=true\&useSSL=false\&serverTimezone=Asia/Ho\_Chi\_Minh
...
baseURL: import.meta.env.VITE\_API\_URL || "http://localhost:8000"
